...
class LiDARCommonSurfelPoint
{
public:
  LiDARCommonSurfelPoint(std::shared_ptr<LiDARModel> lidar, Eigen::Vector3d lidar_point, double* plane,
                         double timestamp, double map_time, double huber_loss, double weight)
    : lidar_(lidar)
  ...
  LiDARCommonSurfelPoint(std::shared_ptr<LiDARModel> lidar, Eigen::Vector3d lidar_point, double* plane,
                         double timestamp, double map_time, double huber_loss)
    : LiDARCommonSurfelPoint(lidar, lidar_point, plane, timestamp, map_time, huber_loss, 1.0)
  {
  }

  LiDARCommonSurfelPoint(std::shared_ptr<LiDARModel> lidar, Eigen::Vector3d lidar_point, double* plane,
                         double timestamp, double map_time)
    : LiDARCommonSurfelPoint(lidar, lidar_point, plane, timestamp, map_time, 5.)
  {
  }

  template <typename TrajectoryModel, typename T>
  Eigen::Matrix<T, 1, 1> reprojectPoint2map(const type::Trajectory<TrajectoryModel, T>& trajectory,
                                            const type::LiDAR<LiDARModel, T>& lidar, const T* plane_cp) const
  {
    int flags = trajectories::EvaluationFlags::EvalPosition | trajectories::EvaluationFlags::EvalOrientation;

    // 'S0' is for cooridinate of trajectory, 'M' is for LiDAR map.
    auto T_MtoS0 = trajectory.Evaluate(T(map_time_) + lidar.time_offset(), flags);
    // 'Sk' is the local cooridate (of trajectory sensor) at `timestamp_`
    auto T_SktoS0 = trajectory.Evaluate(T(timestamp_) + lidar.time_offset(), flags);  // 'l' is for 'lidar local'

    const Eigen::Matrix<T, 3, 1> p_LinS = lidar.relative_position();
    const Eigen::Quaternion<T> q_LtoS = lidar.relative_orientation();

    Eigen::Matrix<T, 3, 1> p_Lk = lidar_point_.cast<T>();
    // p_Sk = T_LtoS * p_Lk
    Eigen::Matrix<T, 3, 1> p_Sk = q_LtoS * p_Lk + p_LinS;

    // p_M = T_S0toM * T_S0_Sk * p_Sk
    Eigen::Matrix<T, 3, 1> p_M =
        T_MtoS0->orientation.conjugate() * (T_SktoS0->orientation * p_Sk + T_SktoS0->position - T_MtoS0->position);
...
protected:
  ...
  struct Residual
  {
    Residual(const LiDARCommonSurfelPoint<LiDARModel>& m) : measurement(m)
    {
    }
    ...
    const LiDARCommonSurfelPoint& measurement;
  ...
  void AddToEstimator(kontiki::TrajectoryEstimator<TrajectoryModel>& estimator)
  {
    ...
    lidar_->AddToProblem(estimator.problem(), { { map_min_time, map_max_time }, { tmin, tmax } }, residual->lidar_meta,
                         parameters);
